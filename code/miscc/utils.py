...
def compute_discriminator_wgan_loss(netD, real_imgs, fake_imgs, gpus, conditions, lam):
    ...
    if cfg.CPU:
        ...
        fake_logits = nn.parallel.data_parallel(netD.get_cond_logits, inputs, gpus)
    print("real_logits shape = {}".format(real_logits.shape))
    real_logit = real_logits.mean()
    fake_logit = fake_logits.mean()
    print("wgan: fake logit {}, real_logit {}".format(fake_logit, real_logit))
...
def compute_discriminator_loss(netD, real_imgs, fake_imgs,
                               ...
                               conditions, gpus, use_wrong_data=True):
